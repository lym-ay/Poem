...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
    ...
    return YES;
#else
    NSSLog(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    NSSLog(@"Closing leaked statement");
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            NSSLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
- (int)busyRetryTimeout {
    NSSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSSLog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSSLog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        NSSLog(@"error on rekey: %d", rc);
        NSSLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    NSSLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSSLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                NSSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                NSSLog(@"DB Query: %@", sql);
                NSSLog(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                NSSLog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                NSSLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    NSSLog(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    NSSLog(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
        NSSLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            NSSLog(@"Error calling sqlite3_step. Query was interrupted (%d: %s) SQLITE_INTERRUPT", rc, sqlite3_errmsg(_db));
            NSSLog(@"DB Query: %@", sql);
        }
    }
        ...
        if (SQLITE_ERROR == rc) {
            if (_logsErrors) {
                NSSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
                NSSLog(@"DB Query: %@", sql);
            }
        }
            ...
            // uh oh.
            if (_logsErrors) {
                NSSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
                NSSLog(@"DB Query: %@", sql);
            }
        }
        else {
            // wtf?
            if (_logsErrors) {
                NSSLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
                //NSSLog(@"DB Query: %@", sql);
            }
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            NSSLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            NSSLog(@"DB Query: %@", sql);
        }
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        NSSLog(@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    if (self.logsErrors) NSSLog(@"%@", errorMessage);
    return NO;
#endif
}
