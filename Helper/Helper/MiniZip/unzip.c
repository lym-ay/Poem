...
extern int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, uint32_t len)
{
    ...
    s->pfile_in_zip_read->stream.next_out = (uint32_t*)buf;
    s->pfile_in_zip_read->stream.avail_out = (uint32_t)len;

    if (s->pfile_in_zip_read->raw)
    {
        if (len > s->pfile_in_zip_read->rest_read_compressed + s->pfile_in_zip_read->stream.avail_in)
            s->pfile_in_zip_read->stream.avail_out = (uint32_t)s->pfile_in_zip_read->rest_read_compressed +
                s->pfile_in_zip_read->stream.avail_in;
    }
    else
    {
        if (len > s->pfile_in_zip_read->rest_read_uncompressed)
            s->pfile_in_zip_read->stream.avail_out = (uint32_t)s->pfile_in_zip_read->rest_read_uncompressed;
    }
    ...
    while (s->pfile_in_zip_read->stream.avail_out > 0)
    {
        if (s->pfile_in_zip_read->stream.avail_in == 0)
        {
            ...
            s->pfile_in_zip_read->stream.next_in = (uint32_t*)s->pfile_in_zip_read->read_buffer;
            s->pfile_in_zip_read->stream.avail_in = (uint32_t)(bytes_not_read + total_bytes_read);
        }
